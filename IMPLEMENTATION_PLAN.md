...
| 1.5 | AI on/off, output level, model selector, audio-device selector | ✅ **verified on hardware 2026-08-20.** AI on/off, output level and device selector already worked; the model selector was exercised end to end with `mrt2_base` downloaded on the M2 Pro — disabled-while-absent, `TOO SLOW HERE` labelling, the `LOAD ANYWAY?` two-press confirmation, the red MODEL row after loading, and cancelling the confirmation by changing the selection. `core::ModelCatalog` carries upstream's table (50 checks) |
...
| 2.9 | Song Map harmony mode | 🟡 core **tested** — `ChordParser` (475 checks), `SongMapPlayer` (106), `SoundingChord` (21), section tempo editing. Engine wired: section changes reload the chart, the clock ticks off the 50 Hz timer, chord changes move only what changed, PANIC silences it and release restores it, `NEXT CHORD` / `PREVIOUS CHORD` are bindable. UI written 2026-08-20: HARMONY selector on the song, per-section CHORDS and TEMPO fields with unreadable symbols named by position, and the chart on the stage screen. **Not compiled, never run**. Chord length is per section and persisted (schema version 2, `KNOWN_ISSUES.md` §30) |
