...
| 2.7 | Setlists | 🟡 core done, **tested** (`Setlist`, `SetlistController`, 22 tests; `SetlistEditor`, 22 tests). A setlist **screen** now exists so a set can be built, reordered and saved in-app rather than by hand-writing a file. Screen **runs on hardware 2026-08-20**; adding a song to an empty set crashed on first use and is fixed and re-verified (`KNOWN_ISSUES.md` §28). **The save/quit/reopen round trip has still never been run against a real file** |
